rants_berichten <- function(data,colors){
  ...
  plot <- rants_by_user %>% 
    ...
    labs(title = "Frustracie Rewind 2021: Wie gebruikt de FrustraCie om te (anti)ranten?",x="Totaal berichten", y = "Totaal (anti)rants") +
    scale_x_log10(limits=c(70*11.83,1500*11.83),
                       labels = c(1000,2000,5000,10000,15000),
                       minor_breaks = c(seq(1000,10000,1000),12500,15000),
                       breaks = c(1000,2000,5000,10000,15000)) +
    scale_y_log10(limits=c(70,1500),
                       breaks = c(100,200,500,1000,1500), 
                       minor_breaks = c(seq(100,1000,100),1250,1500), 
                       labels = c(100,200,500,1000,1500)) +
    geom_line(mapping=aes(x=x,y=y),
              data.frame(x=c(70*11.83,1500*11.83),y=c(70,1500)),
              inherit.aes = FALSE) +
    coord_fixed(ratio=1) +
