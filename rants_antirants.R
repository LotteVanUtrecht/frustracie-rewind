...
rants_antirants <- function(data,colors){
  ...
  rants_by_user$vjust[rants_by_user$author %in% c("Reinout","Jeroen","Rosanne","Niem")] <- 1.25
  
  plot <- rants_by_user %>% 
    ggplot(aes(x=n,y=rant_frac,color=author)) +
    geom_point(show.legend = FALSE) +
    geom_text(aes(label=author,vjust=vjust),hjust=0, show.legend = FALSE) +
    labs(title = "Frustracie Rewind 2022: Rants & Antirants",x="Aantal (anti)rants", y = "Aandeel rants") +
    geom_hline(yintercept=0.5,linetype="dashed") +
    geom_text(aes(y=mean_rants,x=100,label="Gemiddelde"),hjust=0, vjust=-1,show.legend = FALSE,color="black",size=3) +
    geom_hline(yintercept=mean_rants,linetype="dotted") +
    scale_x_continuous(limits=c(100,1500),
                  labels = c(100,200,500,100,1000,1500),
                  minor_breaks = c(seq(100,1000,by=100),1500),
                  breaks = c(100,200,500,100,1000,1500),trans="sqrt") +
    scale_y_continuous(limits=c(0.37,0.82),
                       breaks = seq(0.4,0.8,by=0.1), 
                       minor_breaks = seq(0.45,0.8,by=0.05), 
